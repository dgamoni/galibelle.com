...
class Tribe_Image_Widget extends WP_Widget {
	...
	public function admin_setup() {
		define( 'CORE_PLUGINS_URL_image_widget', CORE_PLUGINS_URL . '/image-widget/' );
		wp_enqueue_media();
		wp_enqueue_script( 'tribe-image-widget', CORE_PLUGINS_URL_image_widget. 'resources/js/image-widget.js', array( 'jquery', 'media-upload', 'media-views' ), self::VERSION );
	...
	private function get_image_html( $instance, $include_link = true ) {
		...
		if ( !empty( $instance['imageurl'] ) ) {
			...
			// dgamoni
			if ($size == 'large') {
				$params_news_img = array( 'width' => 216, 'height' => 62 );
				$output .= '<img src="'.bfi_thumb( $attr['src'], $params_news_img  ).'" class="">';
			} else if ($size == 'full') {
				$params_news_img = array( 'width' => 510, 'height' => 240 );
				$output .= '<img src="'.bfi_thumb( $attr['src'], $params_news_img  ).'" class="">';
			} else {
